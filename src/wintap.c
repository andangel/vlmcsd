...
static void parseTapArgument(char* argument)
{
	...
	{
		printerrorf("Fatal: %s 不是有效的IPv4地址\n", szIpAddress);
		exit(VLMCSD_EINVAL);
	}
	...
	{
		printerrorf("Fatal: /%s 不是有效的CIDR掩码 /8 and /30\n", szMask);
		exit(VLMCSD_EINVAL);
	}

	if (!((DhcpLeaseDuration = timeSpanString2Seconds(szLeaseDuration))))
	{
		printerrorf("Fatal: 没有在选项中指定的有效时间范围 -%c.\n", 'O');
		exit(VLMCSD_EINVAL);
	}
	...
	if (IpAddress <= Network || IpAddress + 1 >= Broadcast)
	{
		...
		printerrorf("Fatal: 对于此子网，IPv4地址必须为 ");
