...
int server_main(int argc, CARGV argv)//程序从这里开始
{
	global_argc = argc;
	global_argv = argv;
...
#	ifdef _NTSERVICE
	...
	{
		IsNTService = FALSE;//如果不把程序设成服务
		return newmain();//如果不安装服务从这里开始
	}
...
#	if !defined(NO_RANDOM_EPID) || !defined(NO_CL_PIDS) || !defined(NO_INI_FILE)
	...
	memset(KmsResponseParameters, 0, sizeof(KmsResponseParam_t) * MIN_CSVLK);//memset因某些编译器分配空间时，内存中默认值并不为0
...
#	if _WIN32 || __CYGWIN__
	InitializeCriticalSection(&logmutex);//程序调度
