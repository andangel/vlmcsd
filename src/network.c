...
int listenOnAllAddresses()
{
	...
	{
		printerrorf("Fatal: 端口必须是1到65535之间的数字 .\n");
		exit(VLMCSD_EINVAL);
	}
...
static SOCKET network_accept_any()
{
	...
	status = select((int)maxSocket + 1, &ListeningSocketsList, NULL, NULL, NULL);//到这里跳出窗口 监听端口

	if (status < 0) return INVALID_SOCKET;//如果监听返回值 返回INVALID_SOCKET
...
#if defined(USE_THREADS) && (defined(_WIN32) || defined(__CYGWIN__)) // Windows Threads 线程
